...
int main() {
	...
	for (;;) {
		...
		if (CAPTURING == mode && imagePoints.size() >= (size_t)s.nrFrames) {
		...
		if (view.empty()) {
			// if calibration threshold not reached yet, calibrate now
		...
		if (found) {
			// improvew the found corners' corrdinate accuracy for chessboard
			...
			if (mode == CAPTURING && (s.inputCapture.isOpened() || clock() - preTimestamp > s.delay * 1e-3 * CLOCKS_PER_SEC)) {
...
static double computeReprojectionErrors(const vector<vector<Point3f> >& objectPoints, const vector<vector<Point2f> >& imagePoints,
	const vector<Mat>& rvecs, const vector<Mat>& tvecs,	const Mat& cameraMatrix, const Mat& distCoeffs,	vector<float>& perViewErrors, bool fisheye) {
...
static bool runCalibration(Settings& s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs, vector<vector<Point2f> > imagePoints,
